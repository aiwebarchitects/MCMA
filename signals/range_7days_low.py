"""
7 Days Low Range Buyer Signal Generator.
Generates BUY signals when price is within a specified range of the 7-day low.
Uses 1-hour candles to calculate 7-day (168 hours) low and high.
"""
...
class Range7DaysLowSignalGenerator:
    """
    7 Days Low Range-based signal generator.
    
    Strategy:
    - Fetches 7 days of 1-hour candles (168 candles)
    - Calculates 7-day low and high
    - Generates BUY signal when current price is within range of 7-day low
    - Range is defined by: 7day_low * (1 + long_offset) to 7day_low * (1 + long_offset + tolerance)
    """
    
    def __init__(self, long_offset_percent: float = -1.0, tolerance_percent: float = 2.0):
        """
        Initialize 7 Days Low Range signal generator.
        
        Args:
            long_offset_percent: Offset from 7-day low in percent (default: -1.0%)
            tolerance_percent: Range tolerance in percent (default: 2.0%)
        """
        self.long_offset_percent = long_offset_percent
        self.tolerance_percent = tolerance_percent
        self.name = "range_7days_low"
    ...
    def _fetch_candles(self, coin: str, limit: int = 168) -> Optional[pd.DataFrame]:
        """
        Fetch 1-hour candle data from Binance API for 7 days.
        
        Args:
            coin: Coin symbol (e.g., "BTC", "ETH")
            limit: Number of candles to fetch (default: 168 for 7 days)
    ...
    def _calculate_buy_range(self, low_7days: float) -> tuple:
        """
        Calculate buy range based on 7-day low.
        
        Args:
            low_7days: 7-day low price
        ...
        buy_range_low = low_7days * (1 + long_offset)
        buy_range_high = low_7days * (1 + long_offset + tolerance)
    ...
    def generate_signal(self, coin: str) -> Optional[Signal]:
        """
        Generate trading signal for a coin based on 7-day low range.
        ...
        """
        try:
            # 1. Fetch 7 days of candle data (168 hours)
            df = self._fetch_candles(coin, limit=168)
            if df is None or len(df) < 168:
                logger.warning(
                    f"{self.name}: Insufficient data for {coin} "
                    f"(got {len(df) if df is not None else 0}/168 candles)"
                )
                return None
            ...
            # 3. Calculate 7-day low and high
            low_7days = df['low'].min()
            high_7days = df['high'].max()
            
            # 4. Calculate buy range
            buy_range_low, buy_range_high = self._calculate_buy_range(low_7days)
            ...
            signal = Signal(
                ...
                metadata={
                    ...
                    '7days_low': round(low_7days, 6),
                    '7days_high': round(high_7days, 6),
